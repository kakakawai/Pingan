...
def DataProcess(data):
    ...
    tripDiffData = tripData[["SPEED","LONGITUDE","LATITUDE"]].diff()
    #del(data)
    #gc.collect()
    ...
    trainData = pd.concat([TimeCost,lowSpeedTime,speedMax,speedMean,speedMin,speedStd,diffSpeedMean,
                           ...
                           LongitudeMax,suddenBrake,lowSpeedRate,tripLong,
...
def process():
    ...
    print("[+]LinearR" + str(selector.ranking_))
    #############################################

    model = ensemble.RandomForestRegressor(n_estimators=11, n_jobs=2)
