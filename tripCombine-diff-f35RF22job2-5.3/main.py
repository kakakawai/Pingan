...
def DataProcess(data):
    ...
    tripDiffData = tripData[["SPEED","HEIGHT","DIRECTION","LONGITUDE","LATITUDE"]].diff()
    del(data)
    gc.collect()

    tripDiffData = pd.concat([TERandTRIPID, tripDiffData],axis=1)
    ...
    DirMean = tripData["DIRECTION"].mean().fillna(0)
    DirMax = tripData["DIRECTION"].max().fillna(0)
    DirMin = tripData["DIRECTION"].min().fillna(0)
    DirStd = tripData["DIRECTION"].std().fillna(0)
    ...
    trainData = pd.concat([TimeCost,lowSpeedTime,speedMax,speedMean,diffSpeedMean,
                           ...
                           suddenBrake,tripLong,lowSpeedRate,LongitudeMax,
                           LongitudeMin,LongitudeStd,LatitudeMax,LatitudeMin,LatitudeStd,
                           DirMax,DirMean,DirMin,DirStd,
                           centerLongitude,centerLatitude,heightMean,heightStd,callstate], axis=1)
